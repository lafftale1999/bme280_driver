 ...
 *
 * @param handle Handle to the BME280 device.
 * @return Always returns 0. Updates internal `json_string` in handle.
 */
int bme280_parse_json(const bme280_handle_t handle)
{
    handle->json_parsed = true;
    const char* keys[BME280_JSON_KEYS_LEN] = BME280_JSON_KEYS;
    char buf[BME280_JSON_BUF_SIZE];
    
    snprintf(handle->json_string, BME280_JSON_STR_MAX_LEN, "{");

    for(size_t i = 0; i < BME280_JSON_KEYS_SIZE; i++) {
        if (strcmp(keys[i], BME280_JSON_TEMP_KEY) == 0) {
            snprintf(buf, sizeof(buf), "\"%s\":%u", keys[i], handle->temperature);
        }
        else if(strcmp(keys[i], BME280_JSON_PRESS_KEY) == 0) {
            snprintf(buf, sizeof(buf), "\"%s\":%d", keys[i], handle->pressure);
        }
        else if(strcmp(keys[i], BME280_JSON_HUM_KEY) == 0) {
            snprintf(buf, sizeof(buf), "\"%s\":%d", keys[i], handle->humidity);
        }


        strncat(handle->json_string, buf, BME280_JSON_STR_MAX_LEN - strlen(handle->json_string) - 1);
        if (i < BME280_JSON_KEYS_SIZE - 1) {
            strncat(handle->json_string, ",", BME280_JSON_STR_MAX_LEN - strlen(handle->json_string) - 1);
        }
    }
 ...
 */
int bme280_calibration(const bme280_handle_t handle)
{
    ...
    if (i2c_read_data(&handle->i2c_address, &t_p_param_start, buf, BME280_TEMP_PARAMS_LEN + BME280_PRESS_PARAMS_LEN) != 0) {
        BME280_LOG("Unable to read temperature and pressure calibration values\n");
        return 1;
    }
    
    uint8_t hum_param[2] = {
        BME280_HUM_CALIB_FIRST,
        BME280_HUM_CALIB_CONTINUE
    };

    // read humidity calibration values
    if (i2c_read_data(&handle->i2c_address, &hum_param[0], &buf[24], BME280_HUM_CALIB_FIRST_LEN) != 0 ||
        i2c_read_data(&handle->i2c_address, &hum_param[1], &buf[25], BME280_HUM_CALIB_CONTINUE_LEN) != 0) {
        BME280_LOG("Unable to read humidity calibration values\n");
        return 1;
    }
 ...
 */
int bme280_convert_pressure(const bme280_handle_t handle)
{
    ...
    if (var1 == 0) {
        BME280_LOG("Failed to convert pressure. Division by 0.\n");
        return 1;
    }
 ...
 * @return 0 on success.
 */
int bme280_convert_temperature(const bme280_handle_t handle)
{
    int32_t var1, var2;
 ...
 * @return 0 on success.
 */
int bme280_convert_humidity(const bme280_handle_t handle)
{
    int32_t v_x1;
 ...
 */
int bme280_convert_measurements(const bme280_handle_t handle)
{
    if (bme280_convert_temperature(handle) != 0) {
        BME280_LOG("Unable to convert temperature\n");
        return 1;
    }

    if (bme280_convert_pressure(handle) != 0) {
        BME280_LOG("Unable to convert pressure\n");
        return 1;
    }

    if (bme280_convert_humidity(handle) != 0) {
        BME280_LOG("Unable to convert humidity\n");
        return 1;
    }
